...
export function SparkleButton({
  ...
  // Randomise particle positions once on mount
  useEffect(() => {
    if (!penRef.current) return;
    const random = (min: number, max: number) =>
      Math.floor(Math.random() * (max - min + 1) + min);
    penRef.current
      .querySelectorAll<HTMLElement>(".spk-particle")
      .forEach((p) => {
  ...
  return (
    <div className={`spk-wrapper${isActive ? " spk-active" : ""} ${className}`}>
      ...
      >
        ...
        {/* Button label */}
        <span className="spk-text">{children}</span>
      </button>

      {/* Floating particles */}
      <span aria-hidden="true" className="spk-particle-pen" ref={penRef}>
        {Array.from({ length: 18 }).map((_, i) => (
          <svg
            key={i}
            className="spk-particle"
            viewBox="0 0 15 15"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d={PARTICLE_PATH} strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        ))}
      </span>
    </div>
